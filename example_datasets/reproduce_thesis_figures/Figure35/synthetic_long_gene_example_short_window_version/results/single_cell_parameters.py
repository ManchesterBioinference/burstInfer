# -*- coding: utf-8 -*-
"""
Created on Mon Nov 16 20:29:12 2020
...
ms2_signals = genfromtxt('synthetic_fluorescent_traces_w13_short_version.csv', delimiter=',', skip_header=1)  # TODO
...
PERMITTED_MEMORY = 16  # Number of allowed compound states
...
max_likelihood_estimate_array = np.array(max_likelihood_estimate)
seed_setter = int(max_likelihood_estimate.iloc[0,0])
...
transitions[0,0] = max_likelihood_estimate.iloc[1,0]
transitions[1,0] = max_likelihood_estimate.iloc[2,0]
transitions[1,1] = max_likelihood_estimate.iloc[3,0]
transitions[0,1] = max_likelihood_estimate.iloc[4,0]
learned_parameters['A'] = transitions
pi0 = np.ones((1,2))
pi0[0,0] = max_likelihood_estimate.iloc[5,0]
pi0[0,1] = max_likelihood_estimate.iloc[6,0]
learned_parameters['pi0'] = pi0
v = np.ones((2,))
v[0,] = max_likelihood_estimate.iloc[7,0]
v[1,] = max_likelihood_estimate.iloc[8,0]
learned_parameters['v'] = v
learned_parameters['noise'] = float(max_likelihood_estimate.iloc[9,0])
learned_parameters['logL'] = max_likelihood_estimate.iloc[10,0]
learned_parameters['EM seed'] = seed_setter

posterior_traces = demoHMM.get_promoter_traces(learned_parameters, n_steps, n_traces, PERMITTED_MEMORY,
                                               eps, seed_setter)

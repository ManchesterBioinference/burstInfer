...
#selector = 40
#selector = 26
selector = 10 # This is for the totally redone Figure 3.5
...
plt.figure(1)
plt.plot(index, selected_fitted.flatten(), color='blue', lw=1.5)
...
#time_ticks = np
plt.text(0.05, 150, 'x$10^{3}$', size=18, color='black')
#text(0.1, 0.9,'matplotlib', ha='center', va='center', transform=ax.transAxes)
plt.ylim(-100, 200)
plt.savefig('short_synthetic_fit35L.pdf', dpi=300)
plt.show()
...
plt.figure(2)
plt.step(index, selected_promoter.flatten(), color='blue', lw=1.5)
...
plt.savefig('short_synthetic_promoter35L.pdf', dpi=300)
plt.show()
...
plt.figure(3)
n, bins, patches = plt.hist(test_list, 10, density=False, facecolor='b', alpha=0.75)

plt.ylabel('Number of promoter traces')
plt.xlabel('Agreement?')
plt.title('SHORT GENE')
plt.xlim(0.6, 1)
plt.savefig('promoter_agreement35L.pdf', dpi=300)
plt.show()
...
#text(0.1, 0.9,'matplotlib', ha='center', va='center', transform=ax.transAxes)
plt.ylim(-100, 200)
plt.savefig('short_synthetic_fit_usable_long35L.pdf', dpi=300)
plt.show()
...
plt.figure(11)
plt.step(index, selected_promoter.flatten(), color='blue', lw=1.5)
plt.step(index, selected_original_promoter.flatten(), c='black', alpha=0.5, lw=2.5)
plt.xlabel('Time into nuclear cycle 14 (min)')
plt.ylabel('Inferred Promoter State')
#time_ticks = np
plt.xticks(np.arange(0,35,5))
plt.yticks(np.arange(0, 2), labels=['OFF', 'ON'])
plt.savefig('short_synthetic_promoter_usable_long35L.pdf', dpi=300)
plt.show()
...
plt.ylabel('Number of promoter traces')
#plt.xlabel('Agreement?')
plt.xlabel('100 * |$\sum (Promoter_{Inferred} \equiv Promoter_{Original}) / T$|')
plt.xlim(0.6, 1)
plt.savefig('promoter_agreement_usable_long35L.pdf', dpi=300)
plt.show()
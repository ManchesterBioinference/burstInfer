# -*- coding: utf-8 -*-
"""
Created on Fri May  8 17:51:37 2020
...
centre = pd.read_csv('uwt_e1_mutant_comparison_xi_centre_TRC.csv')
inter = pd.read_csv('uwt_e1_mutant_comparison_xi_inter_TRC.csv')
outer = pd.read_csv('uwt_e1_mutant_comparison_xi_outer_TRC.csv')

collected = pd.concat([outer, inter, centre], axis = 0)
#collected = collected.iloc[33:,1:]
collected = collected.iloc[:,1:]
viewer = collected.copy()
...
centre_signals = pd.read_csv('uwt_e1_mutant_comparison_xi_centre_signals_TRC.csv')
inter_signals = pd.read_csv('uwt_e1_mutant_comparison_xi_inter_signals_TRC.csv')
outer_signals = pd.read_csv('uwt_e1_mutant_comparison_xi_outer_signals_TRC.csv')

centre_just_signals = centre_signals.iloc[:,8:]
inter_just_signals = inter_signals.iloc[:,8:]
outer_just_signals = outer_signals.iloc[:,8:]

centre_signals_array = np.array(centre_just_signals)
inter_signals_array = np.array(inter_just_signals)
outer_signals_array = np.array(outer_just_signals)

new_mean_list_centre = []
for i in np.arange(0, len(centre_signals_array)):
    sig = centre_signals_array[i,:]
    sig2 = sig[~np.isnan(sig)]
    new_mean_list_centre.append(np.mean(sig2))
    
new_mean_list_inter = []
for i in np.arange(0, len(inter_signals_array)):
    sig = inter_signals_array[i,:]
    sig2 = sig[~np.isnan(sig)]
    new_mean_list_inter.append(np.mean(sig2))

new_mean_list_outer = []
for i in np.arange(0, len(outer_signals_array)):
    sig = outer_signals_array[i,:]
    sig2 = sig[~np.isnan(sig)]
    new_mean_list_outer.append(np.mean(sig2))
    
new_mean_centre_df = pd.DataFrame(new_mean_list_centre)
new_mean_inter_df = pd.DataFrame(new_mean_list_inter)
new_mean_outer_df = pd.DataFrame(new_mean_list_outer)

new_mean_collected = pd.concat([new_mean_outer_df, new_mean_inter_df, new_mean_centre_df])

# Second big change
collected.iloc[:,2] = new_mean_collected
collected.dropna(axis=0,how='any',inplace=True)

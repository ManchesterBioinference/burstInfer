# -*- coding: utf-8 -*-
"""
Created on Fri Mar 27 21:11:44 2020
...
max_likelihood_estimate = results_dataframe.iloc[17,:]
...
# =============================================================================
# export_df.to_csv('uwt_e1_mutant_comparison_xi_centre_TRC.csv')
# pd.DataFrame(reconstituted_posterior).to_csv('uwt_e1_mutant_comparison_xi_centre_posterior_TRC.csv')
# pd.DataFrame(reconstituted_signals).to_csv('uwt_e1_mutant_comparison_xi_centre_signals_TRC.csv')
# =============================================================================

#%%

ninety = reconstituted_posterior[:,90]
ninety_one = reconstituted_posterior[:,91]
ninety_two = reconstituted_posterior[:,92]

ninety_sum = np.sum(ninety) / len(ninety)
ninety_one_sum = np.sum(ninety_one[~np.isnan(ninety_one)]) / len(ninety_one[~np.isnan(ninety_one)])
ninety_two_sum = np.sum(ninety_two[~np.isnan(ninety_two)]) / len(ninety_two[~np.isnan(ninety_two)])

print(ninety_sum)
print(ninety_one_sum)
print(ninety_two_sum)

middle_viewer = np.sum(reconstituted_posterior[:,65][~np.isnan(reconstituted_posterior[:,65])]) / len(reconstituted_posterior[:,65][~np.isnan(reconstituted_posterior[:,65])])
...
def get_p_zz(initialised_parameters, n_steps, n_traces, signal_struct, compound_states, K,
    ...
    #%%
            ...
            for i in np.arange(0,4):
                ...
                if key_transition == off_off:
                    ...
                    off_off_local.append(xi_result)
                elif key_transition == off_on:
                    off_on_container.append(xi_result)
                    off_on_local.append(xi_result)
                elif key_transition == on_off:
                    on_off_container.append(xi_result)
                    on_off_local.append(xi_result)
                elif key_transition == on_on:
                    on_on_container.append(xi_result)
                    on_on_local.append(xi_result)
            
            xi_count = 1
            
            meta_local.append(off_off_local)
            meta_local.append(off_on_local)
            meta_local.append(on_off_local)
            meta_local.append(on_on_local)
            xi_meta_container.append(meta_local)
    #%%        
            # Expansion Xi        
    ...
    #%%
            ...
            p_zz_singleTrace = np.zeros((K*2, trace_length-1)) # TODO
            for t in np.arange(0, trace_length-1):
                #print(t)
                #print(len(xi_meta_container[t][3]))
                
                if len(xi_meta_container[t][0]) == 0:
                    xi_meta_container[t][0] = np.NINF
                if len(xi_meta_container[t][1]) == 0:
                    xi_meta_container[t][1] = np.NINF
                if len(xi_meta_container[t][2]) == 0:
                    xi_meta_container[t][2] = np.NINF
                if len(xi_meta_container[t][3]) == 0:
                    xi_meta_container[t][3] = np.NINF
                
                p_zz_singleTrace[0,t] = scipy.special.logsumexp(xi_meta_container[t][0])
                p_zz_singleTrace[1,t] = scipy.special.logsumexp(xi_meta_container[t][1])
                p_zz_singleTrace[2,t] = scipy.special.logsumexp(xi_meta_container[t][2])
                p_zz_singleTrace[3,t] = scipy.special.logsumexp(xi_meta_container[t][3])
            p_zz[i_tr] = p_zz_singleTrace
